...
public class DuplicateOrderTests
{
    ...
    public async Task ImportAsync_SameFixtureTwice_RejectsDuplicatesWithoutChangingOriginalOrders()
    {
        ...
        var second = await ImportTestSupport.ImportFixtureAsync(
            service,
            "valid-multi-order-batch.pdf"
        );

        Assert.All(
            second.ImportAttempt.ImportOrderResults,
            result =>
            {
    ...
    public async Task ImportAsync_ConcurrentSameOrder_DatabaseConstraintRejectsRaceLoser()
    {
        ...
        var results = await Task.WhenAll(
            ImportTestSupport.ImportFixtureAsync(
                firstService,
        ...
        : IImportPersistence
    {
        ...
        public void AddImportAttempt(ImportAttempt attempt) =>
            _repository.AddImportAttempt(attempt);

        public void AddOrder(LootSingles.Domain.Orders.Order order) => _repository.AddOrder(order);

        public void DiscardOrder(LootSingles.Domain.Orders.Order order) =>
            _repository.DiscardOrder(order);
