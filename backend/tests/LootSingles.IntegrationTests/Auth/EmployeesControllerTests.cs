...
public class EmployeesControllerTests
{
    ...
    public async Task Manager_CanCreateListAndReadAuditEvents()
    {
        ...
        {
            username = "newpicker", displayName = "New Picker", initialPin = "1234", role = "Picker",
        });
        ...
        Assert.Contains(list!, item => item.EmployeeId == created!.EmployeeId && item.Username == "newpicker");

        var audit = await client.GetFromJsonAsync<List<AuditEventResponse>>(
            $"/api/employees/{created!.EmployeeId}/audit-events");
        Assert.Contains(audit!, item => item.ActionType == "AccountCreated" && item.TargetEmployeeId == created.EmployeeId);
    }
    ...
    public async Task Create_DuplicateReturnsConflictAndMalformedInputReturnsBadRequest()
    {
        await using var factory = new AuthWebApplicationFactory();
        using var client = await LoginAsAsync(factory, EmployeeRole.ManagerAdmin);
        var request = new { username = "newpicker", displayName = "New Picker", initialPin = "1234", role = "Picker" };
        Assert.Equal(HttpStatusCode.Created, (await client.PostAsJsonAsync("/api/employees", request)).StatusCode);

        var duplicate = await client.PostAsJsonAsync("/api/employees", new
        {
            username = "NEWPICKER", displayName = "Duplicate", initialPin = "1234", role = "Picker",
        });
        var malformedPin = await client.PostAsJsonAsync("/api/employees", new
        {
            username = "other", displayName = "Other", initialPin = "12ab", role = "Picker",
        });

        Assert.Equal(HttpStatusCode.Conflict, duplicate.StatusCode);
        Assert.Equal("username_taken", (await duplicate.Content.ReadFromJsonAsync<ErrorResponse>())?.Error);
    ...
    public async Task Manager_CanDeactivateReactivateResetPinAndUnlock()
    {
        ...
        await AssertEmployeeStateAsync(factory, target.Id, isActive: true, isLocked: false, failedCount: 0);

        await SetLockoutAsync(factory, target.Id);
        Assert.Equal(HttpStatusCode.OK, (await client.PostAsJsonAsync(
            $"/api/employees/{target.Id}/reset-pin", new { newPin = "4321" })).StatusCode);
        await AssertEmployeeStateAsync(factory, target.Id, isActive: true, isLocked: true, failedCount: 5);
        ...
        Assert.Equal(HttpStatusCode.OK, (await loginClient.PostAsJsonAsync(
            "/api/auth/login", new LoginRequest("target", "4321"))).StatusCode);
    }
    ...
    public async Task MissingEmployeeActionsReturnNotFound()
    {
        ...
        Assert.Equal(HttpStatusCode.NotFound, (await client.PostAsync("/api/employees/404/deactivate", null)).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await client.PostAsync("/api/employees/404/reactivate", null)).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await client.PostAsJsonAsync(
            "/api/employees/404/reset-pin", new { newPin = "1234" })).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await client.PostAsync("/api/employees/404/unlock", null)).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await client.GetAsync("/api/employees/404/audit-events")).StatusCode);
    }
    ...
    public async Task PickerReceivesForbiddenFromEveryEmployeeEndpoint()
    {
        ...
        var requests = new[]
        {
            ...
            {
                Content = JsonContent.Create(new { username = "new", displayName = "New", initialPin = "1234", role = "Picker" }),
            },
    ...
    private static async Task<HttpClient> LoginAsAsync(AuthWebApplicationFactory factory, EmployeeRole role)
    {
        ...
        var response = await client.PostAsJsonAsync("/api/auth/login", new LoginRequest(username, "1234"));
        ...
        AuthWebApplicationFactory factory, string username, string pin, EmployeeRole role)
    {
        ...
        {
            Username = username, NormalizedUsername = username.ToUpperInvariant(), DisplayName = username,
            PinHash = hasher.Hash(pin), Role = role, CreatedAt = DateTimeOffset.UtcNow,
        };
    ...
    private static async Task AssertEmployeeStateAsync(
        AuthWebApplicationFactory factory, int employeeId, bool isActive, bool isLocked, int failedCount)
    {
        using var scope = factory.Services.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<LootSinglesDbContext>();
        var employee = await context.Employees.AsNoTracking().SingleAsync(item => item.Id == employeeId);
