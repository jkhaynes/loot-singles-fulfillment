...
var outputDir = args.Length > 0
    ...
    : Path.GetFullPath(Path.Combine(
        Directory.GetCurrentDirectory(), "..", "..", "tests", "LootSingles.Fixtures", "PackingSlips"));
...
string BuildDescription(string set, string productName, string collectorNumber, string rarity, string condition) =>
    $"Pokemon - {set}: {productName} - {collectorNumber} - {rarity} - {condition}";

var fixtures = new (string FileName, PackingSlipDocument Document)[]
{
    ("invalid-quantity.pdf", new PackingSlipDocument(
    [
        new OrderPageSpec("INVALID-QUANTITY-FIXTURE",
        [
            new ProductLineSpec("0",
                BuildDescription(validSet, validProductName, validCollectorNumber, validRarity, validCondition),
                "$1.00", "$0.00"),
            new ProductLineSpec("-1",
                BuildDescription(validSet, validProductName, validCollectorNumber, validRarity, validCondition),
                "$1.00", "-$1.00"),
            new ProductLineSpec("abc",
                BuildDescription(validSet, validProductName, validCollectorNumber, validRarity, validCondition),
                "$1.00", "$0.00"),
        ]),
    ])),

    ("missing-set.pdf", new PackingSlipDocument(
    [
        new OrderPageSpec("MISSING-SET-FIXTURE",
        [
            // Set left empty (only the separating colon remains) so OrderLineExtractor parses an empty Set.
            new ProductLineSpec("1",
                BuildDescription(string.Empty, validProductName, validCollectorNumber, validRarity, validCondition),
                "$1.00", "$1.00"),
        ]),
    ])),

    ("missing-collector-number.pdf", new PackingSlipDocument(
    [
        new OrderPageSpec("MISSING-COLLECTOR-FIXTURE",
        [
    ...
    ("missing-product-name.pdf", new PackingSlipDocument(
    [
        new OrderPageSpec("MISSING-PRODUCT-FIXTURE",
        [
            // Product name left empty after the last colon in the set/product segment.
            new ProductLineSpec("1",
                BuildDescription(validSet, string.Empty, validCollectorNumber, validRarity, validCondition),
                "$1.00", "$1.00"),
        ]),
    ])),

    ("missing-condition.pdf", new PackingSlipDocument(
    [
        new OrderPageSpec("MISSING-CONDITION-FIXTURE",
        [
            // "Unknown" isn't in ConditionVariantParser.KnownConditions, so Condition resolves to null.
            new ProductLineSpec("1",
                BuildDescription(validSet, validProductName, validCollectorNumber, validRarity, "Unknown"),
                "$1.00", "$1.00"),
        ]),
    ])),

    ("partial-batch-one-bad-order.pdf", new PackingSlipDocument(
    [
        new OrderPageSpec("PARTIAL-BATCH-VALID-1",
        [
            new ProductLineSpec("1",
                BuildDescription(validSet, validProductName, validCollectorNumber, validRarity, validCondition),
                "$1.00", "$1.00"),
        ...
        new OrderPageSpec(null,
        [
            new ProductLineSpec("1",
                BuildDescription(validSet, validProductName, validCollectorNumber, validRarity, validCondition),
                "$1.00", "$1.00"),
        ]),
        new OrderPageSpec("PARTIAL-BATCH-VALID-2",
        [
            new ProductLineSpec("2",
                BuildDescription(validSet, "Mewtwo ex", "#068/086", validRarity, "Lightly Played"),
                "$2.00", "$4.00"),
        ]),
    ])),
};
