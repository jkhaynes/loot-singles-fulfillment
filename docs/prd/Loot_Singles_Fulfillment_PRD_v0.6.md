...
**Changes in 0.6:** amendment A17, approved 2026-09-22, arising from
feature 019 (automated stage and production deployment). Hosting (§40.8)
now serves the web application and the API from **one origin per
environment**, from the same container, superseding 0.5's use of Azure
Static Web Apps. The session cookie is `SameSite=Strict`, and a browser
does not attach a `Strict` cookie to a request originating from a
different site, so a separate origin for the web application would break
authentication outright; the alternative — relaxing the cookie — would
weaken a credential control to satisfy a hosting arrangement. Azure
Container Apps and Azure SQL Database are unchanged, as is every other
part of §40. No product behaviour changes.
...
designed for the desktop. Picking is the mobile-first workflow; manager
review is not.
...
Choosing an order **opens** it. Claiming is an explicit action on the
order itself, so that viewing an order is always safe and starting one
is always the same act. Pick Next Order remains a single action that
claims and opens the next order.

## Already Holding an Order

When an employee already holds a claim, the dashboard must offer to
**resume** that order rather than offering to start another.

Holding an order is a state the interface should reflect, not an error
it should report after the fact.
...
## 12.1 Advancing Never Records

In the focused view, moving between products --- by swipe or by control
--- must **not** record any outcome. Only an explicit action records a
pick or an issue.

Navigation controls must be operable without swiping.

Swipe-to-advance and swipe-to-confirm are one gesture apart, and the
second would violate §23. The distinction is stated here so it is not
eroded during implementation.
...
When every product in a set is resolved, the application **must**
present the transition explicitly, naming the next set and its size:
...
Sets must be grouped by **game**, because Loot stores each game in its
own section and crossing between sections is the expensive walk. Within
a game, sets are ordered alphabetically.

The game is carried on the order line itself (§33), so grouping by it
relies on authoritative order data rather than catalog enrichment.

Loot's shelves run newest-to-oldest within a section, so release-date
ordering would match the aisle more closely. It is deliberately **not**
a V1 requirement: the order data carries no release date, and adding one
means widening the catalog lookups per game and defining where an
undated set sorts. Ordering within one game's aisle only decides which
direction the picker walks it, so the gain does not yet justify the
work.

Release ordering may be added later without disturbing anything built on
this rule.

Unlike an uncertain image (§17), an imperfect sort is not a safety
...
Progress must be expressed in **physical cards** as well as products,
and must include the picker's position within the current set:

> Product 3 of 5\
> 6 of 9 physical cards accounted for\
> Surging Sparks --- 2 of 3
...
Advancing never records an outcome (§12.1).
...
Where a picker finds a **different card** that appears to explain the
discrepancy --- a different variant, condition or set of the same
product --- the issue must be able to record that card's identity
alongside the ordered one, and note that it has been set aside with the
order.

The most common real case is an inventory mismatch: the ordered
non-holo is missing, an unlisted holo is present, and the holo was
almost certainly entered as the non-holo. The picker already sets that
card aside with the order. Without somewhere to record it, the evidence
survives only as prose in an optional note, and the manager reviewing it
reads a problem report rather than a proposal.
...
## 20.1 Completing the Lifecycle

In 0.3, an order in `Needs Attention` could only leave by someone
finding the card. When the card genuinely is not there, the order was
stuck permanently. These states resolve that.

**Awaiting Customer Decision**

An order whose issue has been put to the customer. It is not actionable
by a picker and must be visually distinct from issues that still need
attention.

It must show how long it has been waiting, so a manager can see which
conversations have gone quiet. Nothing expires or cancels automatically
--- a refund is a decision about a customer's money, and it stays with a
person.

**Packed**

An order whose sleeve has been packed and dispatched. This is the
terminal state.

Without it, `Picked` is terminal, completed work accumulates
indefinitely, and there is no record of what is still physically on the
shelf.

**Cancelled**

An order the customer has had refunded in full. It stops appearing in
working views.
...
A product line may also end as:

-   **Written off** --- the customer accepted a refund for it and the
    rest ships
-   **Substituted** --- a different card ships with the customer's
    approval, recorded against the line

An order containing a written-off line is complete but **short**, and
must be represented as short wherever it is packed.

Customer contact and the recording of these outcomes are manager-only
(§9.4).
...
The dashboard must show **counts** per state. Detail belongs on the page
dedicated to that state, which the count links to.
...
Inline per-order detail grows without bound as the queue grows ---
precisely when the dashboard most needs to stay readable. Identifying
which lines are flagged, without opening the order, is served by the
dedicated page rather than the dashboard tile.
...
Every pick **must** end on a completion screen. There are two endings,
...
In 0.3 a pick ended with a status label changing and nothing else, and
a bundle with an unresolved issue had no defined resting place.

## 22.1 Order Hand-off and Labelling

Picked cards are sleeved together and set aside; packing happens later,
by a different person. Nothing in 0.3 tied a sleeve of cards to the
order it belonged to, so the bundle became anonymous the moment it left
the picker's hand.

Every completed pick must produce a printed label identifying the order,
applied to the sleeve.

The label must carry, in human-readable form:
...
A label for an order with an unresolved issue must be visually distinct
**in monochrome** --- an inverted band, not a colour --- and must state
the held-aside count.

An order that ships short (§20.2) must be labelled as short.
...
printing is necessarily client-side. The label is sized for standard
address stock (about 1⅛ × 3½ inches) on the shop's small label printer.

**Boundary.** This section covers **association** --- which order is
this? --- and not **verification** --- are its contents correct?
Packing verification remains a V2 goal (§36) and a V1 non-goal (§35).
...
-   Issue reporter
-   Relevant timestamps
-   Packer, and when it was packed
-   For a substituted line (§20.2): what was ordered, what shipped in
    its place, and that the customer approved it

A substitution changes what physically ships. If it is not recorded,
nobody can answer a customer question about it later.
...
Generating physical order barcodes was a non-goal in 0.3. It was
removed by Product Owner decision on 2026-09-20 after the owners
confirmed label printers are already on site; labelling is now §22.1.

Packing **verification** remains a non-goal, deliberately: V1
identifies a bundle, it does not check its contents.

------------------------------------------------------------------------

# 36. Future V2: Packing

A future version may address packing errors.

Order **association** --- generating an identifier, attaching it to the
bundle, scanning it at the packing desk, and recording who packed it ---
moved into V1 as §22.1.

What remains V2 is scan-based **verification** of a bundle's contents
against its order, and the packing workflow built on it:
...
V1 answers "which order is this?". V2 would answer "are its contents
correct?".
...
The web application and the API are served from **one origin per
environment**, by the same container. The session cookie is
`SameSite=Strict`, and a browser does not attach a `Strict` cookie to a
request originating from a different site, so a separate origin for the
web application would break authentication outright. Serving both from
one origin keeps the cookie control intact; the alternative — relaxing
the cookie — would weaken a credential protection to satisfy a hosting
arrangement.

This supersedes 0.5's use of Azure Static Web Apps (amendment A17,
approved 2026-09-22). Azure Container Apps and Azure SQL Database are
unchanged.

Each environment is isolated: its own database, its own identities, its
own retained logs. Neither environment can read or modify the other's
data.
...
**Substantially answered** by the 2026-09-20 discovery session. The shop
contacts the customer, who chooses between a full refund, a refund of
the missing item with the rest shipped, or an approved substitute.
Managers own that conversation; pickers set candidate cards aside at
pick time. See §19.1, §20.1 and §20.2.
...
**Prototyped** in the 2026-09-20 discovery session, which covered every
item above and produced amendments A1--A13.

This should be validated with people who actually pull Loot orders
before implementation details become fixed. That validation has **not**
yet happened and remains outstanding.
