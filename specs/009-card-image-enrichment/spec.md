...
- Q: For V1, how should a game's catalog provider handle a packing-slip `Set` value that doesn't exactly, literally match the provider's set name (e.g. TCGplayer's Pokémon `Set` text `"SV: Black Bolt"` vs the provider's set name `"Black Bolt"`)? This resolves PRD §41 Q29. → A: Before querying, strip a recognized series-abbreviation prefix (e.g. `"SV: "`, `"SV10: "`, `"ME05: "`) from the start of the `Set` text when present, using a small, Product Owner-supplied table of series abbreviations (e.g. `"SV"` → `"Scarlet & Violet"`) that only needs updating when Pokémon starts an entirely new naming era (infrequent — not per new set release, unlike a per-set/per-group table); query using the remaining text after stripping. When no known abbreviation prefix is recognized, query the raw `Set` text unchanged exactly as today. This is internal to the catalog provider only — used solely to build a better provider query — and does not add, persist, or display any new field on the order line. Capturing Series/Set Number as their own persisted, displayed order-line fields (e.g. for a product like `"ME05: Pitch Black"` → series "Mega Evolution", set number "5") is real, useful information but is explicitly out of scope for this feature and deferred to a separate future feature.
- Q: The Pokémon TCG API (PRD §31's leading candidate for Pokémon) was found, via live testing against a real 50-line order, to fail roughly half of all requests regardless of concurrency (confirmed via direct testing: 8 concurrent, 6 sequential one-at-a-time, and 6 sequential with a 1-second pause between each all showed a ~50-65% failure rate) — should Pokémon stay on this provider, switch to a different one, or add resiliency (retries/throttling) around it? → A: Switch the Pokémon provider to TCGdex (`api.tcgdex.net`). The same 50-concurrent-request load that broke the Pokémon TCG API succeeded 50/50 (100%) against TCGdex, at ~250ms per request. No API key required. This changes the provider implementation only (`ICardCatalogProvider`'s contract, `CardImageEnrichmentService`, and every other consumer are unaffected, per Constitution Principle IX's replaceable-integrations design) — see research.md §4 for the full evidence and integration-shape differences (TCGdex resolves sets by id, not by name, requiring a one-time-per-request sets-list lookup).
- Q: After switching to TCGdex, manual testing of the same real 50-line order still showed only a few images resolving. Evaluating every failing line against TCGdex directly surfaced three distinct causes beyond the already-known set-name-prefix and provider-choice issues above — collector-number leading zeros, an unrecognized-prefix set name, and a trailing printing/variant descriptor in the product name — how should each be handled? → A: (1) **Leading zeros**: always strip them before querying TCGdex's collector-number path — confirmed live that TCGdex inconsistently rejects the zero-padded form per-set (e.g. Celebrations, Evolving Skies reject `"028"`/`"005"` but accept `"28"`/`"5"`) while others accept both, so stripping is safe everywhere observed. (2) **Unrecognized-prefix set names** (e.g. `"Celebrations: Classic Collection"`, whose prefix `"Celebrations"` is itself a real standalone set name, not a short series-era code): expand normalization to also try a colon-to-space fallback candidate (`"Celebrations Classic Collection"`, confirmed to match TCGdex's real name) when the abbreviation table doesn't recognize the prefix — this is now handled, not merely documented as an accepted gap. (3) **Trailing printing/variant descriptors** (e.g. `"Alakazam V (Full Art)"` vs the provider's plain `"Alakazam V"`): strip a trailing parenthetical from the imported product name for the *name-verification comparison only* — the displayed, authoritative `ProductName` is never altered, only what's compared against the provider's card name; this reuses the same parenthetical pattern `OrderLineExtractor` (feature 001) already recognizes as a printing/variant marker, not a general fuzzy match. All three are implementation-level corrections within feature 009's existing provider, not scope changes — see research.md §4.
- Q: After feature 012 (the `OrderLineExtractor` duplicate-collector-number-echo fix) merged in, re-evaluating the same real order's remaining non-matches found every promo-card line (e.g. Sobble, `"ME: Mega Evolution Promo"`; three Scarlet & Violet lines including Charizard ex, `"SV: Scarlet & Violet Promo Cards"`; one Sword & Shield line, `"SWSH: Sword & Shield Promo Cards"`) still fails to resolve, even though its series-abbreviation prefix *is* recognized. Confirmed live against TCGdex's full sets list: TCGdex names these sets `"MEP Black Star Promos"`, `"SVP Black Star Promos"`, and `"SWSH Black Star Promos"` — a distinct naming convention (`"<abbreviation-based code> Black Star Promos"`) that today's abbreviation-stripping normalization never produces (it strips the prefix and keeps TCGplayer's own trailing `"Promo"`/`"Promo Cards"` wording instead). How should this be handled? → A: Extend `PokemonSeriesPrefixNormalizer.NormalizeCandidates` so that, only when a known abbreviation prefix *was* recognized and the resulting stripped set name has the evidenced trailing shape (`"... Promo"` or `"... Promo Cards"`), it also offers two additional candidates built directly from that same already-recognized abbreviation: `"{abbreviation} Black Star Promos"` and `"{abbreviation}P Black Star Promos"`. Both forms are needed because TCGdex's real convention is inconsistent per era — confirmed live: plain form for DP/HGSS/BW/XY/SM/SWSH (`"SWSH Black Star Promos"`), a `P`-appended form for SV/ME (`"SVP Black Star Promos"`, `"MEP Black Star Promos"`) — and, exactly like every other candidate this method already produces, a candidate is only ever used if it exactly matches a real entry already present in TCGdex's own fetched set-name dictionary, so an incorrect guess for some future, unevidenced era simply fails to match and falls back to the placeholder rather than risking a wrong image. This is an implementation-level correction confined to `PokemonSeriesPrefixNormalizer`, not a scope change — see research.md §4 (promo-set-naming addendum). Implemented and verified live against the real order (Pecharunt, Terapagos ex now resolve); Sobble and Charizard ex still correctly show the placeholder for the separate, already-documented reason that TCGdex has no `image` field for those two specific cards.
- Q: TCGdex's own API guidance states "no published hard rate limits, but please be considerate — for bulk data needs, cache responses locally rather than fetching the same data repeatedly." Auditing this feature's actual call pattern found the sets list (218 sets, ~35KB, changes only a handful of times a year) is refetched in full on every single order-detail page load — the cache introduced by the provider switch (research.md §4) lives on the Scoped `TcgdexCardCatalogProvider` instance, which is created fresh per HTTP request, so it only de-duplicates concurrent lookups *within* one page load, never *across* separate page loads or pickers. How should this be brought in line with TCGdex's guidance? → A: Promote the sets-list cache out of the per-request provider into a new app-lifetime singleton, `TcgdexSetCatalog`, shared across every request, with a 24-hour cache duration so a newly-released TCGdex set is still picked up within a bounded window without requiring a redeploy. `TcgdexCardCatalogProvider` delegates to it instead of caching the sets list itself; the underlying `HttpClient` is still obtained per-fetch via `IHttpClientFactory` (not held long-lived), so normal handler rotation/DNS-refresh behavior is unaffected. This is purely an in-memory, non-persisted cache of upstream *reference* data (which set name maps to which TCGdex set id) — it does not cache, persist, or reuse any resolved card match or image URL between requests, so it does not revisit or weaken research.md §11's "no caching or persistence" decision about enrichment *results*; every order-detail request still performs a live per-card lookup and gets a live match. Caching individual per-card lookups (`GET /sets/{setId}/{collectorNumber}`) is a related but separate, smaller optimization, tracked as a follow-up GitHub issue rather than bundled into this fix.
...
- **Card Catalog Source**: A conceptual, game-specific external source of card/printing information and imagery. Each source serves exactly one game in this phase: TCGdex (Pokémon — switched from the Pokémon TCG API per the 2026-08-25 clarification above), Scryfall (Magic: The Gathering), Lorcast (Disney Lorcana). One Piece Card Game has no designated source in this phase.
...
- The three phase-1 games' catalog sources are TCGdex (Pokémon), Scryfall (Magic: The Gathering), and Lorcast (Disney Lorcana). Pokémon started on the Pokémon TCG API per the PRD's original provider research (PRD §31) and switched to TCGdex per the 2026-08-25 clarification above, once live testing found the original choice unreliable under real order volumes. One Piece Card Game is explicitly deferred: a suitable catalog source has not yet been selected for it (PRD §31, §42 Priority 1), and product lines for it continue to show the existing neutral placeholder.
