...
- [X] T049 [P] Run backend build, full unit suite, full SQL Server integration suite, and CSharpier; confirm all existing tests remain green and record results in `specs/009-card-image-enrichment/validation-results.md` — build clean, CSharpier clean (159 files), 179/179 unit, 107/107 integration
- [X] T050 [P] Run frontend lint, unit test suite, production build, Prettier check, and the full Playwright suite; confirm all existing tests remain green and record results in `specs/009-card-image-enrichment/validation-results.md` — lint clean (1 pre-existing unrelated warning), 45/45 unit, build clean, Prettier clean, 12/12 Playwright
- [X] T051 Perform the quickstart.md manual validation (run the app locally with real outbound internet access, exercise every scenario including at least one real confident match per game against the live providers and a simulated provider-unavailable case) and record observations in `specs/009-card-image-enrichment/validation-results.md` — all 6 scenarios validated, including a live simulated Scryfall outage (temporarily pointed its base address at an invalid host, confirmed all 46 Magic lines safely showed no image while 39 non-Magic lines resolved normally with zero errors, then reverted and confirmed restored)
- [X] T052 Perform the constitution Architecture and Changeability Review against the implemented boundaries, capture any Must Fix findings as new tasks in `specs/009-card-image-enrichment/tasks.md`, and do not proceed to convergence until resolved — reviewed plan.md's Architecture and Changeability Review table against the actual implementation, corrected a stale prediction (Lorcast was predicted to need no batch override; it does, for concurrency-capping) and added an Update block covering Lorcast's actual design and its two post-implementation fixes; zero Must Fix findings, two Advisory notes recorded in validation-results.md; ready for `/speckit-converge`
