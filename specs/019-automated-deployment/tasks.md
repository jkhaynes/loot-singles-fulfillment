...
- [X] T019 [US1] [MANUAL] Create both database users and grant their roles — `db_datareader`/`db_datawriter` for the app identity, plus `db_ddladmin` for the migrate identity, **neither `db_owner`** (FR-021) — **runbook: Part 2 C11**. **Grants done 2026-09-23** on both databases, and the temporary firewall rules removed (the portal's query editor re-adds one on every visit — Part D). **Outstanding**: confirmation that the split is actually correct, which is what `/health/database` returning 200 proves on the first deployment. The migrate job is no longer run by hand here — `deploy-stage.yml` points it at the commit's image, runs it, and fails the deploy if it fails. **Stage proven 2026-09-23**: the first stage deploy's migrate run succeeded under the migrate identity, and the first sign-in (C17) read the Employees table and saved a Data Protection key under the app identity — both sides of the split work. **Production proven 2026-09-23**: the first production release's migrate run succeeded, its smoke test's `check /health/database 200` passed — the application identity reading its own database — and the first manager signed in.
- [ ] T020 [US1] [MANUAL] Create the first manager account on the **migrate** identity and change the PIN at first sign-in (FR-032). Re-sequenced: this now runs **after** the first successful deployment, as a one-off execution of the already-deployed image's `bootstrap-admin` command, rather than as a throwaway job created beforehand. Same identity, same requirement, one fewer resource to create and delete. **Runbook gap found 2026-09-23**: this task pointed at C10, which is the migrate job — the runbook has no bootstrap step at all, so FR-032 is uncovered there. Writing one before the flow has ever been run is how the nine portal defects got in; it is written as part of T045, from an actual run. **Stage done 2026-09-23**: the first manager account was created on stage and signed in, and the runbook step now exists as **C17**, written from that run. **Production 2026-09-23**: account created after the first production release, and signed in. **Outstanding**: confirmation that the production PIN was changed at first sign-in — the temporary one is recorded in the job's run history until it is.
- [ ] T021 [US1] [MANUAL] After the first **production** release, sign in **on a phone** over the public address, claim an order and record a pick — the first real write through the application identity (SC-001). The hand-deployment this task originally opened with is now done by `deploy-production.yml`; the verification is the part that matters and is unchanged. **2026-09-23**: the first production release ran and the first manager signed in; no orders exist on production yet, so the claim-and-pick half cannot happen until the first real import.
- [X] T022 [US1] [MANUAL] Configure the custom domain: a `CNAME` for the chosen subdomain and the `TXT` ownership record in Namecheap, leaving the storefront's records untouched; confirm the certificate is issued and the address loads securely (FR-003) — **runbook: Part 2 C15**. Unaffected by the re-sequencing, and deliberately left until after the first deployment. **Done 2026-09-23**: production is **https://fulfillment.lootcardshop.com** and stage is **https://fulfillment-test.lootcardshop.com** (the stage address is a Product Owner choice — FR-003 requires only production's). DNS stays on Namecheap's own nameservers; the shop owner added the CNAME and `asuid.` TXT for each, and the storefront's records were not touched. Both bound with free Azure-managed certificates (production's valid to 2027-03-23, renewed automatically), and both answer `/health` 200 and serve the web app over HTTPS. The `…azurecontainerapps.io` addresses keep working, and the deploy workflows still use them.
...
- [ ] T027 [US2] [MANUAL] Verify end to end: merge a pull request with a visible change and confirm it reaches stage with **zero** manual actions (SC-002); then merge a change that breaks the quality checks and confirm stage is **not** updated. **First half proven 2026-09-23**: merging PR #36 (`2450acc`) deployed to stage with no manual action — gate, build, migrate, update, smoke test, rollback skipped. **Outstanding**: the second half, a deliberately failing change that must not reach stage.
...
- [X] T037 [US5] [MANUAL] Confirm in **both** environments that application log lines reach the Log Analytics workspace and are searchable there, not merely visible in the live stream (FR-027, SC-006) — **runbook: Part 2 Part D**. **Done 2026-09-23**: queried from the shared workspace, 24 hours retained — `ca-loot-singles-prod` 382 lines, `ca-loot-singles-stage` 838, plus the migrate jobs' runs, each separable by app name.
- [ ] T038 [US5] [MANUAL] Read a sample of retained records in both environments and confirm they contain no customer PII, PINs, tokens, secrets or connection strings (FR-022). **Half done 2026-09-23**: 24 hours of both environments' logs searched for credential-shaped text — connection strings, passwords, cookies, bearer tokens, the bootstrap settings — found four lines, all benign (two expected `XmlKeyManager[35]` warnings, two naming a missing setting without its value); the bootstrap logged only "Initial Manager/Admin employee created." **Outstanding**: the customer-PII half, which cannot be checked meaningfully until real orders have been imported.
...
- [X] T040 [P] [MANUAL] Confirm privacy parity between stage and production side by side: both default-deny database access, both split the application and migration identities, both hash PINs, both log packing-slip access (FR-029, SC-010) — **runbook: Part 2 Part D**. **Done 2026-09-23**: Part D run side by side — one subnet rule and no firewall rules on each SQL server, Entra-only auth on both, two separate identities per environment, 7-day restore on both. Both environments run the **same image** (`sha-2450acc…`), so PIN hashing and packing-slip logging are the same code by construction.
...
- [X] T045 Work through quickstart.md end to end as written, correcting anything that does not match what was built. **Pass done 2026-09-23** over everything checkable without a deployment — Parts 1, A, B, C, D, E, F, G read line by line, with Part B's network and database claims verified live against Azure. Corrected: Part B named the production app registration `github-loot-singles-prod` (it is `github-loot-singles-production`) and listed no container image; Part 2's header said to run everything twice, which stopped being true when the environment became shared; **Part E still described one resource group per environment**, the change the 2026-09-23 clarification called for and never received; Part 1's non-root check ran `docker run … whoami`, which with an `ENTRYPOINT` and no `CMD` starts the web server instead of reporting the user, so it could never have failed; three stale cross-references (Part F job logs → C11, T017 → C4–C5, T018 → C6–C7); and **C16 was added** for the GHCR package visibility the runbook never mentioned. **Outstanding**: the bootstrap step for FR-032 (see T020) and the deployment flow itself, neither of which can be written honestly until something has actually deployed. **Update 2026-09-23, after the first deploy**: the bootstrap step now exists as **C17**, from the real run; C16 records that the package came out public on its own; Part F gained rows for a failed bootstrap, the expected `XmlKeyManager[35]` warning, and querying logs without the `log-analytics` extension. **Still outstanding**: the production release flow. **Done 2026-09-23**: the new **Part H — Releasing** documents both releases as they actually ran (stage on merge; production by named full SHA, approval, smoke test, automatic rollback), and **C15** now records the real custom domains and the order that setup took, including the stage record's typo and how the outside check caught it.
