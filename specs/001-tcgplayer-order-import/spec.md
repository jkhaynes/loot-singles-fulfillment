...
- **Import Attempt**: Represents one execution of the import process against a supplied packing slip file. A single packing slip file commonly contains multiple orders (13 observed in one real example). Key attributes: timestamp, whether the file itself could be read at all (FR-015), any discrepancy found against the batch's own order-identifier summary/index page when present (FR-013), and — while still processing — progress counters (total orders detected, orders processed so far, running counts of successes and failures, per FR-014). Contains one or more Import Order Results, one per order the attempt evaluated. Used to answer "how far along is this import?" while it runs and, together with its Import Order Results, "was this packing slip imported, and what happened?" once it's done.
- **Import Order Result**: Represents the outcome for one order within an Import Attempt — kept as its own record rather than folded into the Import Attempt, since an attempt commonly evaluates many orders independently (FR-005). Key attributes: the source order identifier (if it could be read), outcome (succeeded/rejected), the specific rejection reason when rejected, and a reference to the resulting Order when succeeded. Belongs to exactly one Import Attempt.
