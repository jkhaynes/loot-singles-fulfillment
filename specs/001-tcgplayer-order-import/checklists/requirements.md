...
- Post-session revision (same day): FR-006 now requires a stable failure-type code (from a fixed, named set) alongside the human-readable rejection reason, split between per-order codes (Import Order Result) and attempt-wide codes (Import Attempt). All eight codes trace to existing FRs; none are new scope.
- Post-session revision (same day): FR-005 expanded to require card/collector number and condition per Order Line (rejecting the order if missing/unreadable); rarity and variant/printing remain optional. New FR-018 requires condition and variant/printing to be parsed as separate fields even when combined in the source, and requires all variant/printing information to be preserved regardless of where in the description it appears. FR-006's codes extended with `MissingCollectorNumber`/`MissingCondition`; SC-009 added.
- Post-session revision (same day): closed a data-minimization gap — the Assumptions section had left retention of the raw source PDF (which carries customer shipping PII on every page) as an open technical-planning detail. Strengthened FR-009, added FR-019 (source PDF and any copy of it must never be retained beyond a single import operation, regardless of outcome), and extended SC-004 and User Story 3 to verify it.
