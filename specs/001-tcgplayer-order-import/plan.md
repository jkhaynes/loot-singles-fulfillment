...
**Post-convergence correction (2026-08-20)**: `/code-design-review` found that `contracts/import-service.md`'s exception-handling clause, as originally written, conflicted with FR-016 and this plan's own research.md §5 decision for the specific case of a genuine (non-duplicate) persistence failure on one order within a multi-order batch — letting such a failure propagate as an exception would abort the entire `ImportAsync` call and prevent sibling orders from being persisted, directly violating FR-016. `contracts/import-service.md`, `research.md` §5, and `data-model.md` were corrected to state explicitly that every per-order persistence failure — duplicate-key or otherwise — is represented as a typed `ImportOrderResult` rejection, never a propagated exception; exception propagation remains reserved for failures outside the per-order persistence boundary. This does not change the Constitution Check gates above: Principle V (Safe Failure Over Silent Corruption) and XI (Reliability During Fulfillment) are better satisfied by this correction, not weakened by it. The implementation (`LootSinglesDbContext`, `PackingSlipImportService`, `FailureType`, and `AtomicPersistenceTests`) still reflects the pre-correction (T059) behavior as of this plan update and requires a follow-up `/speckit-implement` pass, including a new regression test that combines a multi-order fixture with a persistence-failure interceptor — the coverage gap that let the regression through undetected.

**Post-implementation architecture refinement (2026-08-20)**: a developer design review of `OrderLineExtractor`/`OrderLineValidator` (not a `/code-design-review` Must Fix — both were already spec-compliant and fully tested) found the two used inconsistent idioms for the same kind of outcome: the extractor threw `OrderLineExtractionException` for structural parsing failures while the validator used null-checks for field-presence failures, and the extractor constructed a full `OrderLine` before it was confirmed valid. Per research.md §8, `OrderLineExtractor.Extract` now returns the existing `OrderLineValidationResult` type instead of throwing; the two remain separate responsibilities (parsing vs. business-rule checking), and no new abstraction was introduced — a rule-registry approach was explicitly considered and rejected as speculative, since FR-005's required-field set is closed. This is a behavior-preserving refactor per constitution Principle IV: `FailureType` codes and rejection outcomes are unchanged; the Constitution Check gates above are unaffected (still PASS/N/A).
