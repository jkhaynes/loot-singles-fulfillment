...
- **Does not throw** for ordinary rejection cases (an unreadable file, a malformed order, a duplicate) — those are represented as data in the yielded updates and the final `ImportAttempt`/`ImportOrderResult` records (FR-006), not as exceptions. Exceptions are reserved for genuine infrastructure failures (e.g., database unavailable), which are the caller's problem to handle, not a defined outcome of this contract.
