...
## 8. Order Line extraction/validation failure representation (FR-002, FR-005, FR-006, post-implementation architecture refinement)

**Decision**: `OrderLineExtractor` and `OrderLineValidator` remain two separate responsibilities — structural parsing of the raw description text (splitting segments, locating the collector number, locating the set/product separator) stays in the extractor; field-presence business rules (quantity, blank product name/set/collector number/condition) stay in the validator. What changes is *how* they communicate a failure to each other and to the caller: `OrderLineExtractor.Extract` returns the same `OrderLineValidationResult` record the validator already returns (`OrderLine?`, `FailureType?`, `FailureMessage?`) instead of throwing `OrderLineExtractionException`. The validator calls the extractor, checks `IsValid`, and — on success — continues with its own field-presence checks against the returned candidate `OrderLine`, exactly as before; on failure, it returns the extractor's result unchanged rather than catching an exception and re-wording the message.

**Rationale**: Raised via `/code-design-review` discussion (not a review finding — a developer design concern about the original implementation): the extractor and validator used two different idioms for the same kind of outcome ("this line is bad, here's why") — one via thrown exceptions, one via null-checks — and the extractor constructed a full `OrderLine` domain object before it was confirmed valid. Reusing the existing `OrderLineValidationResult` type (rather than introducing a new, parallel result type) avoids duplicating an already-established shared concept (Constitution XII). Keeping extraction and validation as two separate functions (rather than merging them into one) preserves the existing single-responsibility boundary — parsing vs. business-rule checking — per Constitution XII's default that these stay separate absent a concrete reason to combine them; only the failure-communication mechanism was inconsistent, not the responsibility split itself. This is a refactor, not new behavior: `FailureType` codes and which cases are rejected are unchanged; a small number of individual failure *messages* now come directly from the extractor's own wording instead of being re-worded by the validator's old catch block, since no test or FR pins exact message text (FR-006 requires the message be specific and human-readable, not a particular string) and removing that redundant re-wording step is itself part of what resolves the two-idiom inconsistency.

**Alternatives considered**:
- **Merge extraction and validation into one function** — rejected: would blend "can these fields even be located in the text" with "are the located fields acceptable" into a single responsibility, and the two-idiom inconsistency was in *how failures are communicated*, not in having two functions.
- **A rule-registry/pluggable-validator abstraction for field checks** — rejected: FR-005 already fixes the required-field set (product name, set, collector number, condition, quantity) as closed; rarity and variant are permanently optional. There is no approved or foreseen requirement implying this set will keep growing, so a pluggable-rule abstraction would be speculative (Constitution XIII: abstractions must solve a concrete, foreseen variation point, not a hypothetical one).
- **A new, separate result type for extraction** — rejected in favor of reusing `OrderLineValidationResult`, which already has the exact right shape and is already tested.
