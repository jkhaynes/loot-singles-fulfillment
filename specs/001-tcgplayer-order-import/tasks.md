...
## Phase 10: Architecture Refinement — Order Line extraction/validation idiom (developer design review, research.md §8)

- [X] T064 Change `OrderLineExtractor.Extract` (`backend/src/LootSingles.Application/Import/OrderLineExtractor.cs`) to return `OrderLineValidationResult` instead of throwing `OrderLineExtractionException` for structural parsing failures (too few segments, no collector number match, no set/product separator), reusing the existing `Invalid`-style construction; update `OrderLineValidator.Validate` (`backend/src/LootSingles.Application/Import/OrderLineValidator.cs`) to call the extractor and check `IsValid` instead of try/catch, returning the extractor's result unchanged on failure and continuing with its own field-presence checks (product name/set/collector number/condition) on success exactly as before. Delete `backend/src/LootSingles.Application/Import/OrderLineExtractionException.cs` (no longer referenced). Move `OrderLineValidationResult` to its own file (`OrderLineValidationResult.cs`) since it is now produced by two classes. Update `backend/tests/LootSingles.UnitTests/Import/OrderLineExtractionTests.cs` to unwrap `.OrderLine` from the new return type (mechanical signature change only). Keep `FailureType` codes and rejection outcomes unchanged — this is a behavior-preserving refactor per constitution Principle IV: the full test suite (including `OrderValidationTests.cs`, `RejectionTests.cs`, `ValidImportTests.cs`) must remain green throughout, per research.md §8 and plan.md's "Post-implementation architecture refinement" note (refactor) — resolved: `OrderLineExtractor.Extract` now returns `OrderLineValidationResult`; `OrderLineValidator.Validate` checks `IsValid` instead of try/catch and no longer re-words the extractor's own failure message; `OrderLineExtractionException.cs` deleted; `OrderLineValidationResult` moved to its own file. Confirmed Red (compile error in `OrderLineExtractionTests.cs` against the old signature) → Green. Full suite green (47/47, same count as before — no new tests added, none needed for a behavior-preserving refactor)
- [X] T065 Add a static `Invalid(FailureType type, string message)` factory to `OrderLineValidationResult` (`backend/src/LootSingles.Application/Import/OrderLineValidationResult.cs`) and use it from both `OrderLineExtractor.Extract` and `OrderLineValidator.Validate` (`backend/src/LootSingles.Application/Import/OrderLineExtractor.cs`, `OrderLineValidator.cs`), removing their duplicate private `Invalid` helpers — per code-design-review Advisory #2 from the T064 review pass. Mechanical deduplication with identical behavior, already fully covered by the existing suite; no new test needed (constitution's stated TDD exception for work with no meaningful new behavior to test) — existing tests must remain green throughout (advisory) — resolved: static factory added to `OrderLineValidationResult`; both classes' private `Invalid` helpers removed and call sites updated. Build clean (0 warnings, `TreatWarningsAsErrors` on — confirms no dead code left behind). Full suite green (47/47, unchanged)
- [X] T066 Eliminate the double-parse in `PackingSlipImportService` (`backend/src/LootSingles.Application/Import/PackingSlipImportService.cs`): `ValidateBlock` currently calls `OrderLineValidator.Validate(line)` once per product line to check the order is valid, then `CreateOrder` calls it again on every line (discarding the first pass's results) to build the actual `OrderLine`s — every line is parsed twice per import. Change `ValidateBlock` to collect and return the `List<OrderLineValidationResult>` it already produces while checking validity, alongside the existing nullable `(FailureType, string)` failure tuple, and change `CreateOrder` to take that list and build `OrderLine`s directly from the already-validated results (`result.OrderLine!`) instead of re-calling `OrderLineValidator.Validate`. Update the call site in `ImportAsync` accordingly. Behavior-preserving (same validation order, same failure detection, same constructed `OrderLine`s) — per constitution Principle IV, existing tests (`ValidImportTests.cs`, `RejectionTests.cs`, `PartialImportTests.cs`, `DuplicateOrderTests.cs`, `AtomicPersistenceTests.cs`) must remain green throughout; no new test needed for this mechanical efficiency fix (refactor) — resolved: `ValidateBlock` now returns `(List<OrderLineValidationResult> LineResults, (FailureType Type, string Message)? Failure)`, collecting results as it validates; `CreateOrder` takes that list and builds `OrderLine`s from `result.OrderLine!` directly instead of re-validating. `ImportAsync`'s call site updated to destructure both values. Each product line is now parsed exactly once per import instead of twice. Full suite green (47/47, unchanged)