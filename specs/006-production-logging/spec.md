...
- Q: Should the failure-type breakdown also include each affected order's TCGplayer order identifier (not just a count), so an operator can look the order up directly (e.g., in TCGplayer) without a database lookup? → A: Yes, for per-order failures within an otherwise-readable attempt (validation rejections, duplicates) — include the affected orders' TCGplayer order identifiers alongside each failure type's count, with no cap on how many are listed. This does not apply to the whole-file-unreadable case (FR-005), where no orders were evaluated and there is nothing to identify. An order rejected specifically because its own identifier is missing (`MissingOrderIdentifier`) is represented with an explicit placeholder rather than an empty value.
...
- Multiple duplicate-order rejections within the same batch are ordinary, expected outcomes; they are reflected within the single attempt-level completion log entry's failure-type breakdown (FR-004) — as a count plus the affected orders' identifiers — and MUST NOT each produce their own log entry.
...
- **FR-004**: When an order-import attempt finishes with at least one order failing (but the attempt itself was able to run), the system MUST emit exactly one Warning-level log entry reporting the import attempt's identifier, the counts of orders detected/succeeded/failed, and a breakdown of how many failed orders fall under each failure type present in that attempt (for example, a count of `DuplicateOrder` and a separate count of `MissingProductName`), so the reason for the failures is visible without querying the database. Each failure type in the breakdown MUST also list the TCGplayer order identifier of every order that failed with that type, with no cap on how many are listed, so a specific failed order can be looked up directly without a database query; an order whose own identifier is the thing missing (`MissingOrderIdentifier`) MUST be represented with an explicit placeholder rather than an empty value.
...
- **SC-001**: For any completed import attempt, an operator can determine from console log output alone — without querying the database — how many orders were detected, how many succeeded or failed, and (when applicable) which failure type(s) occurred and the TCGplayer order identifier of each affected order (except when the whole file could not be read, per FR-005, since no orders were evaluated); for an order that failed because of an unexpected persistence problem, the operator can additionally identify that specific order from its own dedicated log entry.
