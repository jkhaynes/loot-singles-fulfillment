...
- [X] T017 [P] Confirm no new logging package or alternate log provider was introduced: grep every `backend/**/*.csproj` for a disallowed logging package name (Serilog, Application Insights/`Microsoft.ApplicationInsights`, Log Analytics, Seq, Datadog) and confirm zero matches, and confirm `backend/src/LootSingles.Api/Program.cs` contains no `ClearProviders()`/`AddProvider()`/third-party logging registration beyond the ASP.NET Core defaults, recording the result in `specs/006-production-logging/validation-results.md`, per FR-001/FR-002
...
- [X] T019 Import a 200-order synthetic batch (reusing the `SyntheticProgressiveParser` pattern from `ProgressReportingTests`) and confirm exactly one attempt-level log entry is produced regardless of batch size, satisfying SC-003 and FR-009's proportionality requirement
- [X] T020 Run backend build, full unit suite, full SQL Server integration suite, and CSharpier; confirm all existing tests remain green (SC-004) and record results in `specs/006-production-logging/validation-results.md`
- [X] T021 Perform the quickstart.md manual validation (run the API locally, exercise the three logged outcomes through the frontend or direct HTTP calls, confirm console output matches expectations and contains no unsafe content) and record observations in `specs/006-production-logging/validation-results.md`
- [X] T022 Perform the constitution Architecture and Changeability Review against the implemented boundaries, capture any Must Fix findings as new tasks in `specs/006-production-logging/tasks.md`, and do not proceed to convergence until resolved

---

## Phase 6: Per-Order Identifiers in the Failure-Type Breakdown

**Purpose**: Extend FR-004/SC-001 (amended per this session's clarification, spec.md) so an operator can look up a specific failed order (e.g., in TCGplayer) directly from the Warning completion log, without a database query — for per-order failures within an otherwise-readable attempt only; the whole-file-unreadable case (FR-005) still has no orders to identify.

- [X] T023 Add a failing test extending the existing duplicate-breakdown scenario to assert the completion log's structured state also contains `DuplicateOrderIds` (comma-joined, all 13 affected orders' `SourceOrderIdentifier` values, uncapped), and a new test using `missing-order-identifier.pdf` asserting `MissingOrderIdentifierCount=1` and `MissingOrderIdentifierIds="(missing)"` (the placeholder for the one failure type whose own identifier is what's missing), in `backend/tests/LootSingles.IntegrationTests/Import/ImportLoggingTests.cs`
- [X] T024 Run T023 and confirm it fails for the expected reason (no `*Ids` field exists in the breakdown today)
- [X] T025 In `LogCompletion`'s breakdown loop in `backend/src/LootSingles.Application/Import/PackingSlipImportService.cs`, add one `{FailureType}Ids` argument per failure-type group alongside its existing `{FailureType}Count`, joining each group's `SourceOrderIdentifier` values with `,` and substituting the literal `(missing)` for any null/whitespace identifier, to make T023 pass
- [X] T026 Run T023, confirm it passes, and run the full backend test suite to confirm no regression; record results in `specs/006-production-logging/validation-results.md`

**Checkpoint**: The failure-type breakdown identifies specific failed orders directly from console output, per this session's clarification.

- [X] T027 During T021's manual validation, observed that the breakdown's free-text message rendered an unlabeled `{count} {ids}` pair (e.g. `1 MISSING-PRODUCT-FIXTURE`) with no indication of which `FailureType` it belonged to, since the console's default formatter only prints the formatted message text, never the named structured arguments separately — failing SC-005's "console output alone" requirement even though the structured state was complete and correct. Fixed by labeling each breakdown group with its failure-type name in the message template (`{FailureType}: {Count} [{Ids}]`, e.g. `DuplicateOrder: 13 [...]`), extending T023's tests to assert the labeled message text, and updating `data-model.md` to require the label. Full regression: 74 unit passed, 100 SQL Server integration passed, CSharpier clean.
