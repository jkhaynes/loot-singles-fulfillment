...
- [X] T025 `/code-design-review` Must Fix: FR-003 in `spec.md` had its clauses inverted ("MUST render that line without a blank or broken rarity field, rather than omitting it"), literally instructing the opposite of the omit-when-absent behavior the Edge Cases section, data-model.md, SC-003, feature 007's analogous FR-009, and the shipped implementation/tests all correctly agree on. Corrected FR-003's wording to mirror FR-009's clause order. No code change needed — the implementation was already correct
- [X] T026 `/code-design-review` Advisory: the order status in `frontend/src/features/orders/OrderDetailPage.tsx` rendered as bare text with no accessible label distinguishing it as a status (a screen reader would hear only the literal value, e.g. "ready", with no context), unlike every per-line attribute which has a `<dt>` label. Added `aria-label={\`Order status: ${order.status}\`}` to the status element, with a Red→Green test in `frontend/tests/orders/OrderDetailPage.test.tsx` asserting the accessible label
