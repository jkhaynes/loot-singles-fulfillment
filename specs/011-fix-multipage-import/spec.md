...
- What happens when a page whose product lines genuinely cannot be extracted for a reason other than the missing continuation-page "Total" row (e.g., its table header can't be located at all)? Unchanged, pre-existing behavior (not something this fix touches, worsens, or is scoped to correct): such a page contributes no block and no rows, exactly as it already does today for any page lacking a locatable table header. This fix corrects specifically the false-positive "no product lines on a continuation page that does have a locatable header but no Total row" case — it does not weaken or change detection of pages that fail for any other reason.
