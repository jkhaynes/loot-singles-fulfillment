...
- [X] T003 [US1] Run the updated `OrderLineExtractionTests.cs` and confirm the new case fails for the expected reason (the current algorithm returns `Invalid`/`MissingProductName` for this no-colon input)
...
- [X] T005 [US1] Run T002, confirm it passes, and run the full `OrderLineExtractionTests.cs` suite (all existing Pokémon and Magic cases) to confirm no regression
...
- [X] T007 [P] [US2] Run the full `backend/tests/LootSingles.IntegrationTests` suite (which exercises the broader import pipeline, including the existing Pokémon fixture PDFs) and confirm no regression
- [X] T008 [US2] Run T006–T007 and confirm they pass immediately; if either reveals a regression, fix `OrderLineExtractor.cs` to resolve it before proceeding
