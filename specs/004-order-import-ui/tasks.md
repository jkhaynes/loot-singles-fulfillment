...
- [ ] T019 [P] [US2] Unit tests in `backend/tests/LootSingles.UnitTests/Orders/OrdersServiceTests.cs` (following `DashboardServiceTests.cs`'s fake-repository pattern): the service returns an empty list when the repository has no orders, and otherwise returns the repository's projected list items unmodified.
...
- [ ] T022 [P] [US2] Create `backend/src/LootSingles.Application/Orders/OrderListItem.cs`: `public sealed record OrderListItem(int OrderId, string TcgplayerOrderId, OrderStatus Status, DateTimeOffset ImportedAt)`, per data-model.md's read projection.
- [ ] T023 [US2] Create `backend/src/LootSingles.Application/Orders/IOrderRepository.cs` with `Task<IReadOnlyList<OrderListItem>> GetAllAsync(CancellationToken cancellationToken)` (depends on T022).
...
- [ ] T031 Run full quickstart.md validation: `dotnet build backend/LootSingles.sln`, `dotnet test backend/LootSingles.sln`, `npm --prefix frontend test`, `npm --prefix frontend run build`, `npm --prefix frontend run test:e2e`; confirm all nine quickstart.md end-to-end scenarios pass.
...
- **User Story 1 (Phase 3)**: Depends on Foundational (T001–T003) completion. No dependency on User Story 2.
- **User Story 2 (Phase 4)**: Depends on Foundational (T001–T003) completion. Independent of User Story 1's implementation, though its E2E task (T030) conveniently reuses T018's sign-in flow.
- **Polish (Phase 5)**: Depends on both user stories being complete.

### Within User Story 1

- Tests (T004–T012) MUST be written and confirmed failing before implementation (T013–T018) begins.
- T013 → T014 → T015 (same file, sequential).
- T016 is independent of T013–T015 (different files/layers).
- T017 depends on T012 (its own failing tests) and T016 (the API it calls).
- T018 depends on T017 (the page it drives) and T003 (E2EHost wiring).
...
- All Foundational tasks (T001, T002, T003) are marked [P] and can run in parallel — different files, no dependencies among them.
- All User Story 1 test tasks (T004–T012) can be written in parallel — 9 independent files.
- All User Story 2 test tasks (T019–T021) can be written in parallel — 3 independent files.
- T024 and T025 (OrdersService and OrderRepository) can be implemented in parallel once T023 exists.
...
2. Complete Phase 3: User Story 1 (T004–T018).
...
4. Polish (T031) → full quickstart.md validation across both stories.
...
- No customer shipping/PII field appears anywhere in DTOs, responses, or UI for this feature (FR-012) — verify this explicitly while implementing T013–T017, T022, T026, T029.
