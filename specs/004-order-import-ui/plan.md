...
**Constraints**: One PDF/request with a server-enforced 25 MB maximum; any authenticated role; synchronous request; propagate HTTP abort cancellation; completed per-order transactions survive interruption; retry cannot duplicate orders; explicit `InProgress`/`Completed`/`Failed` transport status and client-derived `Interrupted`; Failed and Interrupted retain clearly qualified partial results; no customer shipping data; no mobile horizontal scroll
...
| V Safe failure | Typed codes/status remain authoritative; terminal record required; Failed/Interrupted partial results are clearly qualified and never reported as completion | PASS |
...
| XI Reliability | Abort cancels remaining work, preserves committed orders, and supports duplicate-safe retry; missing terminal state becomes Interrupted | PASS |
