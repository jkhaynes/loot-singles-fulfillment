...
- [ ] T002 [P] Record the deferral decision and its mitigation in `specs/017-pick-completion-handoff/research.md` §9 and in `spec.md`'s Risks section
...
**⚠️ No user story may begin until this phase is complete.**
...
- [ ] T016 [P] [US1] Integration tests for `GET /api/orders/{orderId}/label` in `backend/tests/LootSingles.IntegrationTests/Orders/` — success shape, `orderNotFound`, `orderNotPicked`, and that **no customer field appears in the payload** (FR-015)
...
- [ ] T020 [US1] Create `backend/src/LootSingles.Application/Packing/LabelContent.cs` deriving every printed value from the order (FR-017)
...
- [ ] T023 [US1] Create `frontend/src/features/labels/OrderLabel.tsx` rendering text, QR and Code 128, with the Code 128's printed value serving as the human-readable full identifier (FR-012)
...
- [ ] T041 [P] [US2] RTL tests for the packing desk — resolve, refuse a held order by name, state plainly when no slip is stored
...
- [ ] T062 [US3] Add a reprint action to `frontend/src/features/orders/OrderDetailPage.tsx` for any picked order
