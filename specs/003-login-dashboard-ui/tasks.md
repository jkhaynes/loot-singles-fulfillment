...
- [X] T016 [US2] Implement `DashboardRepository` (`IDashboardRepository` over `LootSinglesDbContext`, `AsNoTracking` + server-side `Count`/`Sum` projection per data-model.md) in `backend/src/LootSingles.Infrastructure/Persistence/DashboardRepository.cs` (depends on: T014) — at the time, ordering was applied after materialization, not via SQL `ORDER BY` (SQLite could not translate `DateTimeOffset` ordering; see data-model.md); superseded by 005-sql-server-migration, which restored a SQL-translated `ORDER BY` now that SQLite is no longer used in tests
