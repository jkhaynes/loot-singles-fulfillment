...
- [X] T014 [P] [US3] Add failing integration tests to `backend/tests/LootSingles.IntegrationTests/Auth/EmployeesControllerTests.cs`: `POST /api/employees/{id}/deactivate` returns `409 { "error": "would_remove_last_manager_admin" }` per contracts/employee-management-api.md when it's the last active Manager/Admin
- [X] T015 [US3] Add a failing real-database concurrency test in a new `backend/tests/LootSingles.IntegrationTests/Persistence/EmployeeLastManagerAdminConcurrencyTests.cs`: with exactly two active Manager/Admin employees, two simultaneous `DeactivateAsync` calls — one per employee — never both succeed (exactly one succeeds, the other returns `WouldRemoveLastManagerAdmin`), mirroring the real-concurrency pattern in `BootstrapAdminCommandTests.cs`'s `ExecuteAsync_ConcurrentBootstrapAttemptsOnSameEmptyDatabase_ExactlyOneSucceeds`
- [X] T016 [P] [US3] Add failing frontend component tests to `AdminPage.test.tsx`: a "Remove" action per active-employee row calls `deactivateEmployee` and the row shows inactive afterward; a "Restore" action per inactive-employee row calls `reactivateEmployee` and the row shows active afterward; a mocked `would_remove_last_manager_admin` rejection shows a clear message and leaves the row unchanged

### Implementation for User Story 3

- [X] T017 [US3] In `EmployeeManagementService.DeactivateAsync` (`backend/src/LootSingles.Application/Auth/EmployeeManagementService.cs`), call `SaveChangesGuardingLastManagerAdminAsync` (T003; renamed from the originally-planned `TryGuardLastManagerAdminAsync` during Phase 2 — see that task's note) before applying the deactivation; return `EmployeeManagementResult.WouldRemoveLastManagerAdmin` if it would leave zero active Manager/Admin employees, otherwise proceed as today — depends on T002, T003. **Real bug caught by T013's own tests**: the first implementation called the guard unconditionally, which incorrectly blocked deactivating a *Picker* whenever the store happened to have zero Manager/Admin employees for unrelated reasons — the guard's count query excludes the target by ID regardless of the target's actual role, so it can't tell on its own whether the exclusion is even relevant. Fixed by only invoking the guarded save when the target's current `Role` is `ManagerAdmin`; a non-Manager/Admin target uses the plain `SaveChangesAsync` (deactivating a Picker can never violate the invariant, so no guard is needed).
- [X] T018 [US3] In `EmployeesController.Deactivate` (`backend/src/LootSingles.Api/Controllers/EmployeesController.cs`), map `EmployeeManagementOutcome.WouldRemoveLastManagerAdmin` to `409 Conflict` with `{ "error": "would_remove_last_manager_admin" }` — depends on T017
- [X] T019 [P] [US3] Add `deactivateEmployee(id)`/`reactivateEmployee(id)` functions to `adminApi.ts`, calling the existing `POST /api/employees/{id}/deactivate`/`/reactivate` endpoints and handling the new `409` typed error
- [X] T020 [US3] Add "Remove"/"Restore" actions per roster row in `AdminPage.tsx` (Remove shown for active employees, Restore for inactive), refreshing the roster on success and showing the blocked-removal error inline — depends on T019
...
- [X] T021 [P] [US4] Add failing unit tests to `EmployeeManagementServiceTests.cs`: `ChangeRoleAsync` returns `Success` and updates `Role` for a valid promotion/demotion; returns `NotFound` for a nonexistent employee; returns `WouldRemoveLastManagerAdmin` (leaving the role unchanged) when demoting the last active Manager/Admin; is a no-op `Success` (still recording an audit event) when the requested role matches the employee's current role
- [X] T022 [P] [US4] Add failing integration tests to `EmployeesControllerTests.cs`: `POST /api/employees/{id}/change-role` returns `200` with the updated role, `404` for a nonexistent employee, `400` for a missing/invalid `role` value, `409 would_remove_last_manager_admin` when applicable, and `403` for a non-Manager/Admin caller, per contracts/employee-management-api.md
- [X] T023 [US4] Add failing tests to `backend/tests/LootSingles.IntegrationTests/Auth/SessionInvalidationTests.cs`: extend the synthetic-principal `ValidatePrincipal` test helper to accept a `ClaimTypes.Role` claim, then add `ValidatePrincipal_EmployeeRoleChangedSinceIssuance_RejectsThePrincipal` (cookie's role claim no longer matches the employee's current role) and `ValidatePrincipal_EmployeeRoleUnchanged_AcceptsThePrincipal`; also add a full-endpoint test mirroring `Deactivation_ViaRealEndpoint_InvalidatesAlreadyActiveSessionOnNextRequest`, but changing the target's role via `POST /api/employees/{id}/change-role` instead of deactivating
- [X] T024 [P] [US4] Add failing frontend component tests to `AdminPage.test.tsx`: a role-change control per row calls `changeEmployeeRole` and the row shows the new role afterward; a mocked `would_remove_last_manager_admin` rejection shows a clear message and leaves the role unchanged. Also fixed a genuine collision the new role-change `<select>` introduced in an existing Phase 3 assertion: the Role cell's text ("Picker") and the select's "Picker" option both matched a plain `getByText('Picker')` query, so that one assertion now targets the Role cell by column position instead (mirroring the indexed-cell pattern already used in `DashboardPage.test.tsx`).

### Implementation for User Story 4

- [X] T025 [US4] Add `RoleChanged = 7` to `EmployeeAuditActionType` (`backend/src/LootSingles.Domain/Employees/EmployeeAuditActionType.cs`, data-model.md)
- [X] T026 [US4] Add `ChangeRoleAsync(int actorEmployeeId, int targetEmployeeId, EmployeeRole newRole, CancellationToken)` to `EmployeeManagementService.cs`: look up the target, return `NotFound` if missing; if `newRole` differs from the current role and the current role is `ManagerAdmin`, call `SaveChangesGuardingLastManagerAdminAsync` (T003; see that task's naming note) and return `WouldRemoveLastManagerAdmin` if it would leave zero active Manager/Admin employees, otherwise proceed as today; a no-op (role unchanged) never invokes the guard — otherwise set `Role = newRole`, record a `RoleChanged` audit event (recorded even when `newRole` equals the current role), and save — depends on T002, T003, T025. Applied the same "only guard when the current role is actually ManagerAdmin" fix Phase 5's `DeactivateAsync` needed, from the start this time.
- [X] T027 [US4] Add `POST /api/employees/{id}/change-role` to `EmployeesController.cs` with a `ChangeRoleRequest(string? Role)` body, parsing via `Enum.TryParse<EmployeeRole>` (mirroring `CreateEmployeeRequest.Role`'s existing parsing), mapping outcomes per contracts/employee-management-api.md — depends on T026
- [X] T028 [US4] Extend `EmployeeSessionCookieEvents.ValidatePrincipal` (`backend/src/LootSingles.Infrastructure/Auth/EmployeeSessionCookieEvents.cs`) to also reject the principal when the cookie's `ClaimTypes.Role` claim doesn't match the employee's current `Role`, alongside the existing `!IsActive` check (research.md §2) — depends on T025
- [X] T029 [P] [US4] Add `changeEmployeeRole(id, role)` to `adminApi.ts`, calling `POST /api/employees/{id}/change-role` and handling the new `409`/`400` typed errors
- [X] T030 [US4] Add a role-change control per roster row in `AdminPage.tsx`, refreshing the roster on success and showing the blocked-change error inline — depends on T029
...
- [X] T034 [P] Add `frontend/e2e/admin.spec.ts` covering: create a new employee → see it in the roster → remove it → restore it; and a role change that invalidates a second, already-logged-in session for the promoted/demoted employee — using two authenticated browser contexts, per quickstart.md. The test creates its own throwaway employee(s) via the UI rather than needing new `E2EHost` seed data. Full suite: 16/16 E2E tests passing (up from 14), zero regressions in the other five spec files sharing the same E2EHost/database.
- [X] T035 Run the full manual validation in `quickstart.md` (all 9 scenarios, including the concurrent last-Manager/Admin scenario). Ran against LocalDB (`LootSinglesFulfillment.Dev`) rather than the shared Azure dev database, per explicit instruction — a bootstrapped Manager/Admin plus a throwaway Picker created through the UI gave known, controllable starting state for every scenario. All 9 scenarios passed: roster visibility; Picker denied the admin route; create + case-insensitive duplicate rejection; remove/restore (including confirming login is denied while inactive and resumes unchanged after restore); the last-Manager/Admin guard blocking both a removal (scenario 5) and a role change (scenario 7) with a clear message and no state change; a role change taking effect on a fresh login (the "already-active session is invalidated mid-request" sub-case is the one already proven by T023's `SessionInvalidationTests` and T034's E2E test with genuinely isolated browser contexts — a single shared-cookie browser profile can't hold two simultaneous distinct identities to re-demonstrate that specific mechanic by hand); PIN reset (old PIN rejected, new PIN accepted) and unlock (locked account regains access with its existing, unchanged PIN) — status column correctly rendered "Active - Locked"; and the full audit trail for every action above, correctly attributed to the acting Manager/Admin, including two `RoleChanged` entries. The concurrent-request guarantee itself is T015's automated real-SQL-Server race test (already green), not a manual step — matching quickstart.md's own "Automated validation" section. Encountered and worked around a browser-automation quirk unrelated to the app: coordinate/ref-based clicks were unreliable against this page in this session (confirmed via JS-dispatched `.click()` and direct network/console inspection that the app itself was working correctly throughout); switched to driving the remaining scenarios via `javascript_tool` for reliability.
- [X] T036 Run the full automated validation suite from `quickstart.md` (`dotnet build`, both `dotnet test` projects, `dotnet csharpier check`, `npm run lint/test/build/format:check/test:e2e`) and confirm everything passes with zero regressions. Results: `dotnet build backend/LootSingles.sln` clean; `LootSingles.UnitTests` 216/216; `LootSingles.IntegrationTests` 141/141 (real SQL Server container, ~2m); `dotnet csharpier check backend` clean; `npm run lint` clean (one pre-existing unrelated warning); `npm run test` 66/66; `npm run build` clean; `npm run format:check` clean (after formatting the new `e2e/admin.spec.ts`, which had never been run through prettier before this task); `npm run test:e2e` 16/16.
